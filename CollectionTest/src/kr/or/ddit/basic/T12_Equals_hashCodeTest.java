...
/*
   ...
   검사한다.
    ...
    - eqauls()메서드는 두 객체의 내용(값)이 같은지 비교하는 메서드이고
    ...
    5. hashCOde()는 기본적으로 Heap에 있는 각 객체에 대한 메모리 주소 메핑정보를 
    	기반으로 한 정수값을 반환한다. 그러므로 클래스에서 hashCode()메서드를 override
    	하지 않으면 절대로 두 객체가 같은 것으로 간주 될 수 없다.
    	
    -hashCode()메서드에서 사용하는 '해싱알고리즘'에서 서로 다른 객체에 대해 같은
    hashcode값을 만들어 낼수 있다. 그래서 객체가 같지 않더라도 hashcode가 같을수 있다.
